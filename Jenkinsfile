node('master') {
    ...
    sh 'curl https://api.github.com/repos/bridadan/test_jenkins-pr/statuses/' + parent_commit + '?access_token=' + env.GITHUB_STATUS_TOKEN + ' -H "Content-Type: application/json" -X POST -d \'{"state":"success","target_url":"' + env.JENKINS_URL + build_job_ret.build().getUrl() + '","description":"Job succeeded","context":"Build job"}\''
